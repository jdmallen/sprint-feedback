...
export function getBrowserInfo()
{
	...
	if (/edge\/[0-9]{2}/i.test(navigator.userAgent))
	{
		...
		browser.majorVersion = parseInt(
			/edge\/([0-9]{2})/i.exec(navigator.userAgent)[1],
			10
		);
	...
	{
		const winver = parseFloat(
			/Windows NT ([0-9]{1,2}\.[0-9]{1})/i.exec(navigator.userAgent)[1]
		);
		browser.os = "windows";
		switch (winver)
		{
	...
	{
		browser.os = "os x";
		browser.osversion = /OS X [0-9]{2}_([0-9]{1,2})_[0-9]{1,2}/i.exec(
			navigator.userAgent
		)[1];
	}
